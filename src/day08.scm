(define-module (day08) #:export (count-antinodes))
 ...
 (let* ((arr (make-grid file))
  ...
  (length
   (filter (lambda (pos)
           ...
           (list-distinct (append-map get-group-antinodes antenna-groups))))))
